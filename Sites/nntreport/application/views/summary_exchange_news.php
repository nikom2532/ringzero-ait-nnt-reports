...
<form action="<?php echo base_url()?>index.php/exchange/search" id="formSearch" method="post">
...
<div class="contentOverflow">
  <table style="width:1300px; font-family:'supermarket'; font-size:16px; color:#666" border="0" cellpadding="1" cellspacing="1">
   <thead>
    <tr class="head-bar">
     <th>No</th>
     <th><?php echo $lbl_date?></th>
     <th><?php echo $lbl_organize?></th>
     <th><?php echo $lbl_reporter?></th>
     <th><?php echo $lbl_category?></th>
     <th><?php echo $lbl_news_topic?></th>
     <th><?php echo $lbl_news_title?></th>
     <th><?php echo $lbl_news_status?></th>
     <th><?php echo $lbl_news_read?></th>
 </tr>
</thead>
<tbody>
   <?php
    ...
    $issues = $this->issue_model;
    foreach ($news_list as $val) {
    ?>
    <tr style="background-color:<?php echo ($odd % 2 == 0 ? "#f0f0f0" : "#fff");$odd++;?>">
        <td align="center" width="50"><?php echo number_format($val->RowNumber)?></td>
        <td align="center" width="80"><?php echo $val->newsDate?></td>
        ...
        <td width="150">
            <?php 
            if($val->reporterId!=""){
                $users->setSC03UserId($val->reporterId);
                echo $users->toString();
            }
            ?>
        </td>
        <td>
            <?php 
                if($val->typeId!=""){
                    $newsTypes->setTypeId($val->typeId);
                    echo $newsTypes->toString();
                }
            ?>
        </td>
        <td><?php 
                if($val->issueId!=""){
                    $issues->setIssueId($val->issueId);
...
<ul class="report-box report-box04">
 <li class="num_page clearfix">
   <div class="float_l">
    <span><?php echo $default_total . " : " . $count_row . " " . $default_list . " (" . $total_page . " " . $default_page . ")"?></span>
</div>
...
function nextPage(val) {
  ...
  $("#formSearch").attr("action", "<?php echo base_url()?>index.php/exchange/search/" + nextpage);
  $("#formSearch").submit();
}
function lastPage(val) {
   $("#formSearch").attr("action", "<?php echo base_url()?>index.php/exchange/search/" + val);
   $("#formSearch").submit();
}
function prevPage(val) {
  ...
  $("#formSearch").attr("action", "<?php echo base_url()?>index.php/exchange/search/" + prevpage);
  $("#formSearch").submit();
}
function firstPage() {
   $("#formSearch").attr("action", "<?php echo base_url()?>index.php/exchange/search/1");
   $("#formSearch").submit();
}
...
function exportExcel() {
   $("#formSearch").attr("action", "<?php echo base_url()?>index.php/exchange/export_excel");
   $("#formSearch").submit();
   $("#formSearch").attr("action", "<?php echo base_url()?>index.php/exchange/search");
}

function exportWord() {
   $("#formSearch").attr("action", "<?php echo base_url()?>index.php/exchange/export_word");
   $("#formSearch").submit();
   $("#formSearch").attr("action", "<?php echo base_url()?>index.php/exchange/search");
}

function exportPdf() {
   $("#formSearch").attr("action", "<?php echo base_url()?>index.php/exchange/export_pdf");
   $("#formSearch").submit();
   $("#formSearch").attr("action", "<?php echo base_url()?>index.php/exchange/search");
}

function exportChart() {
   $("#formSearch").attr("action", "<?php echo base_url()?>index.php/exchange/export_chart");
   $("#formSearch").submit();
   $("#formSearch").attr("action", "<?php echo base_url()?>index.php/exchange/search");
