...
<body>
    ...
    <div class="wapper">
    	<div class="header">
            ...
            <?php echo $lbl_welcome?> <span class="blue-txt">นายชื่อ นามสกุล |</span></p><a href="<?php echo base_url()?>index.php/authenticator/signout" class="blue-txt link-logout"><?php echo $lbl_signout?></a></span></div>
        </div>
        ...
        <ul class="report-box report-box02">
			<li class="header-report set-font01 clearfix">
            	<div><span></span></div>
				<div><span><?php echo $lbl_date?></span></div>
    			<div><span><?php echo $lbl_region?></span></div>
    			<div><span><?php echo $lbl_province?></span></div>
    			<div><span><?php echo $lbl_placenews?></span></div>
    			<div><span><?php echo $lbl_qty?></span></div>
			</li>
            <?php foreach($result_array as $val){?>
                <?php foreach($region_list as $regionVal){?>
                    <?php
                        $this->province_model->CM05_RegionID = $regionVal->CM05_RegionID;
                        $result = $this->province_model->findByRegionId(array("Y"));
                        foreach($result['result'] as $provinceVal){
                            $this->news_model->setRegion($regionVal->CM05_RegionID);
                            $this->news_model->setProvince($provinceVal->CM06_ProvinceID);
                            $count = $this->news_model->countNewsByDateAndRegion($val->yearDate);
                    ?>
            <li class="row-report clearfix">
            	<div><span></span></div>
				<div><span><?php echo $val->yearDate?></span></div>
    			<div><span><?php echo $regionVal->CM05_RegionName?></span></div>
    			<div><span><?php echo $provinceVal->CM06_ProvinceName?></span></div>
    			<div><span></span></div>
    			<div><span><?php echo number_format($count);?></span></div>    			
			</li>
            <?php } 
                }
            }?>
